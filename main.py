...
def main_window():
    ...
    def main():
        ...
        # adding country code to number
            ...
            if (st == False):
                ...
                con = mysql.connector.connect(host='localhost', passwd='faiz@123#', user='root',auth_plugin='mysql_native_password')
                cur = con.cursor()

                cur.execute("CREATE DATABASE IF NOT EXISTS data")
                cur.execute("USE data")
                cur.execute("CREATE TABLE IF NOT EXISTS records(Name varchar(20) PRIMARY KEY,Num varchar(15),Time varchar(5),Message varchar(100))")
                s = 'INSERT INTO records VALUES(%s,%s,%s,%s)'
                t = (name,number, time,message)
                cur.execute(s, t)
                con.commit()
                con.close()
            else:
    ...
    def history():
        ...
        con = mysql.connector.connect(host='localhost', passwd='faiz@123#', user='root',database='data',auth_plugin='mysql_native_password')
        cur = con.cursor()
        ...
        style.map('Treeview', background=[('selected', '#125353')])

        tree_frame = Frame(window)
        tree_frame.place(x=5, y=10)
        ...
        # Create Treeview
        trv = ttk.Treeview(tree_frame, yscrollcommand=tree_scroll.set)
        trv.pack()
        ...
        trv.column('5', width=206, anchor='c')

        trv.heading('1', text='S.No')
        trv.heading('2', text='Name')
        trv.heading('3', text='Number')
        trv.heading('4', text='Time')
        trv.heading('5', text='Message')

        trv.tag_configure('odd', background="#a5eccb")
        trv.tag_configure('even', background='#1d8b78')
        my_tag = 'odd'
        ...
        for dt in data:
            ...
            if (my_tag == 'even'):
                ...
                my_tag = 'even'
            trv.insert('', 'end', iid=dt[0], values=(i, dt[0], dt[1], dt[2],dt[3]), tags=(my_tag))

        def remove_one():

            conn = mysql.connector.connect(host='localhost', passwd='faiz@123#', user='root', database='data',auth_plugin='mysql_native_password')
            curr = conn.cursor()
            ...
            trv.delete(x)
            con.close()
            messagebox.showinfo('Info',"Your record has been deleted.")
        ...
        def clear():
            ...
            con.commit()
            con.close()
            messagebox.showinfo('Info','Your history has been cleared!!')

        Button(window, width=20, padx=4, pady=2, text="CLEAR HISTORY", bg='#d5384d', fg="white", font=('Mesquite Std', 9),
               border=0, relief='ridge', borderwidth=1,command=clear).place(x=400, y=300)
        Button(window, width=20, padx=4, pady=2, text="DELETE RECORD", bg='#1971b4', fg="white", font=('Mesquite Std', 9),
               border=0, relief='ridge', borderwidth=1,command=remove_one).place(x=150, y=300)
    ...
    Button(win, width=20, padx=2, pady=2, text="Clear All",bg='#ffdd0b',fg="black", border=0, relief='ridge',borderwidth=1,command=clear).place(x=800, y=590)
    Button(win, width=20, padx=2, pady=2, text="Show History",bg='#ffdd0b',fg="black", border=0, relief='ridge',borderwidth=1,command=history).place(x=610, y=590)
